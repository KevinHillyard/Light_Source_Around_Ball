...
def Spheroid(a, b, resolution=10):
    ...
    spheroid.addNodes(
        [(x + rx * np.sin(n) * np.sin(m), y - ry * np.cos(m), z - rz * np.cos(n) * np.sin(m)) for m in latitudes for n
         in longitudes])

    # Add square faces to whole spheroid but poles
    num_nodes = resolution * (resolution - 1)
    spheroid.addFaces([(m + n, (m + resolution) % num_nodes + n,
                        (m + resolution) % resolution ** 2 + (n + 1) % resolution, m + (n + 1) % resolution) for n in
                       range(resolution) for m in range(0, num_nodes - resolution, resolution)])
...
def FractalLandscape(origin=(0, 0, 0), dimensions=(400, 400), iterations=4, height=40):
    ...
    for i in range(iterations):
        ...
        squares = [(x + y * size, x + y * size + 1, x + (y + 1) * size + 1, x + (y + 1) * size) for y in range(size - 1)
